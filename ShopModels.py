...
class RNN_Shop_MLP(torch.nn.Module):
    def __init__(self,config):
        ...
        assert config['type_hidden_size']+config['distance_hidden_size']+config['price_hidden_size'] +3 == config['shop_mlp_layers'][0]



        self.fc_layers=torch.nn.ModuleList()
        for i in range(len(config['shop_mlp_layers'])-1):
            in_size = config['shop_mlp_layers'][i]
            out_size = config['shop_mlp_layers'][i+1]
            self.fc_layers.append(torch.nn.Linear(in_size,out_size))
        

    def forward(self,type_embedding,distances_embedding,ratings,comments,prices_embedding,group):     
        ratings=ratings.view(ratings.shape[0],ratings.shape[1],1)#变成xx
        comments=comments.view(comments.shape[0],comments.shape[1],1)
        group=group.view(group.shape[0],group.shape[1],1)
        vector=torch.cat([type_embedding,distances_embedding,ratings,comments,prices_embedding,group],dim=-1)
        for idx in range(len(self.fc_layers)):
            vector=self.fc_layers[idx](vector)
            vector=torch.nn.ReLU()(vector)  
        return vector  #c[vj] 怎么解决的？ [batchsize,len(neibor),shop_mlp_layers[-1]]
